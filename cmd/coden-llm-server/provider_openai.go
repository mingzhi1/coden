...
func (p *OpenAI) Chat(ctx context.Context, model string, messages []Message) (string, error) {
	...
	slog.Debug("[llm:openai] sending request", "model", model, "url", p.baseURL+"/chat/completions")
	...
	if resp.StatusCode != http.StatusOK {
		slog.Warn("[llm:openai] error response", "status", resp.StatusCode, "duration_ms", time.Since(start).Milliseconds())
	...
	if reason == "length" {
		return content, &TruncatedError{Content: content, Err: fmt.Errorf("openai: truncated")}
	}

	slog.Info("[llm:openai] response received", "model", model, "reply_len", len(content),
		"tokens_in", result.Usage.PromptTokens, "tokens_out", result.Usage.CompletionTokens)
