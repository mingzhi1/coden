...
func main() {
	serve := flag.String("serve", "", "serve accept worker on this TCP address; default is stdio")
	flag.Parse()

	ctx, stop := signal.NotifyContext(context.Background(), os.Interrupt, syscall.SIGTERM)
	defer stop()

	srv := accept.NewServer(workflow.NewLocalAcceptor())
	...
	fmt.Fprintf(os.Stderr, "CodeN accept worker serving on %s\n", ln.Addr())
	for {
		conn, err := ln.Accept()
		if err != nil {
			if ctx.Err() != nil {
