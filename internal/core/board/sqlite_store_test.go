...
func TestAgent_ListAndUpdate(t *testing.T) {
	s := newTestStore(t)
	for _, id := range []string{"ag-1", "ag-2"} {
		if err := s.CreateAgent(newAgent(id)); err != nil {
			t.Fatal(err)
		}
	}
	...
	agents[0].Status = AgentCoding
	agents[0].UpdatedAt = time.Now().UTC().Truncate(time.Second)
	if err := s.UpdateAgent(agents[0]); err != nil {
		t.Fatalf("update: %v", err)
	}
	got, _ := s.GetAgent(agents[0].ID)
	if got.Status != AgentCoding {
...
func TestAgent_Delete(t *testing.T) {
	s := newTestStore(t)
	if err := s.CreateAgent(newAgent("ag-del")); err != nil {
		t.Fatal(err)
	}
	if err := s.DeleteAgent("ag-del"); err != nil {
		t.Fatalf("delete: %v", err)
	}
	if _, err := s.GetAgent("ag-del"); err == nil {
		t.Error("expected error after delete")
	}
}
