...
type Task struct {
	...
	// SubGoal, when non-empty, marks this task for decomposition into a child
	// workflow instead of a single Code→Accept pass: the kernel recursively runs
	// a Plan→execute pipeline against SubGoal (bounded by maxWorkflowDepth) and
...
type Artifact struct {
	Path     string
	Summary  string
	Evidence []string // execution evidence from the executor (e.g. "go test exit 0")
}
...
type WorkflowContext struct {
	...
	// gathered after planning but before coding. This gives the Executor
	...
	// for inclusion in the Executor's system prompt. Only set for TargetExecutor.
	MCPToolDescriptions string

	// ToolsPrompt holds the dynamically generated "Available tools" section
	// for the Executor's system prompt, replacing the hardcoded tool list.
	...
	// ExecutorMode controls whether the agentic Executor may modify the repo.
	// Zero value (ExecutorModeReadWrite) preserves existing behavior; the kernel
	// sets ExecutorModeReadOnly for read-only intents (e.g. analyze) so the Executor
	...
	// this run, keyed by role name ("analyzer", "planner", "executor", …). It is a
...
const (
	// ExecutorModeReadWrite is the default: the Executor may execute read AND mutation
	// tools (write_file / edit_file / run_shell).
	ExecutorModeReadWrite ExecutorMode = iota
	// ExecutorModeReadOnly restricts the Executor to read tools (read_file / search /
	// list_dir / grep / lsp / rag_search). Mutations are skipped, never executed.
	ExecutorModeReadOnly
)
