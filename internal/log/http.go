...
// NewHTTPClient creates an HTTP client that logs requests/responses when debug
// mode is active. timeout=0 means no timeout.
...
func (h *HTTPRoundTripLogger) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	if slog.Default().Enabled(req.Context(), slog.LevelDebug) {
		slog.Debug(
			...
			"body", truncateBody(bodyToString(save)),
		)
	}
...
const debugBodyLimit = 2048 // bytes shown in debug logs

// truncateBody caps the logged body to debugBodyLimit characters.
func truncateBody(s string) string {
	if len(s) <= debugBodyLimit {
		return s
	}
	return s[:debugBodyLimit] + fmt.Sprintf("... [truncated %d bytes]", len(s)-debugBodyLimit)
}
