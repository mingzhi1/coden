...
// Server exposes the current write-file tool over JSON-RPC.
type Server struct {
...
func NewServer(executor toolruntime.Executor) *Server {
	return NewScopedServer("coden-tool-writefile", "filesystem", "short", []string{"write_file", "read_file", "list_dir", "search", "run_shell", "edit_file", "grep_context"}, executor)
}

func NewScopedServer(name, capability, timeoutClass string, commands []string, executor toolruntime.Executor) *Server {
	if executor == nil {
		panic("writefile executor is required")
	}
	if name == "" {
		name = "coden-tool-writefile"
	}
	if capability == "" {
	...
	if len(commands) == 0 {
		commands = []string{"write_file", "read_file", "list_dir", "search", "run_shell", "edit_file", "grep_context"}
	}
	return &Server{
		executor:     executor,
		name:         name,
